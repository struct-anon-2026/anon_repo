...
def render_markdown(per_project: list[dict], agg: dict) -> str:
    ...
    lines.append('- **GT**：StructAgent full-system 输出经人工 100% 核验')
    ...
    lines.append('- 这是 VLM zero-shot 唯一接近 StructAgent 的维度')
    lines.append('')
    ...
    lines.append('- 与 StructAgent full（B5）对比：B5 在 N=6 上是 100% by definition（GT 即 B5 输出）')
    lines.append('')
    lines.append('---')
    lines.append('')
    lines.append('*生成时间：2026-05-25*')
